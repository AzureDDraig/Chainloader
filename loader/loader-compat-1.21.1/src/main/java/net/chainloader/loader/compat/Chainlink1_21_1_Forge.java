...
public class Chainlink1_21_1_Forge extends Chainlink1_21_1_Base {
    ...
    public void onWakeUp(ClassLoader classLoader) {
        net.chainloader.loader.core.gui.EarlyLoadingScreen.getInstance().log("ForgeCompat", "Initializing 1.21.1 Forge compatibility layer...");
    ...
    private void tryLoadAdapter(String range, String className, List<String> targetVersions, ClassLoader classLoader, List<String> instantiatedClasses) {
        ...
        if (match && !instantiatedClasses.contains(className)) {
            net.chainloader.loader.core.gui.EarlyLoadingScreen.getInstance().log("ForgeCompat", "Detected mod version match for range " + range + ". Loading dynamic subloader: " + className);
            try {
                ...
                System.out.println("[Chainlink 1.21.1 Forge] Successfully registered: " + className);
            } catch (Throwable t) {
                System.err.println("[Chainlink 1.21.1 Forge] Failed to dynamically load adapter " + className + ":");
                t.printStackTrace();
            }
        }
    }
