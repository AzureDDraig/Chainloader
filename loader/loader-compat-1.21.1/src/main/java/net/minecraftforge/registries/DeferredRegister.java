...
public class DeferredRegister<T> {
    ...
    public void register(IEventBus bus) {
        ...
        if (reg == null) {
            System.err.println("[ChainLoader] Warning: Could not resolve registry for Forge DeferredRegister modId=" + modId);
            return;
        }
        ...
        for (Entry<T> entry : entries) {
            try {
                ...
                System.out.println("[ChainLoader] Forge DeferredRegister: Registered " + id + " to registry " + reg.key().location());
            } catch (Throwable t) {
                System.err.println("[ChainLoader] Forge DeferredRegister: Failed to register " + entry.name + " for mod " + modId + ":");
                t.printStackTrace();
            }
        }
    }
